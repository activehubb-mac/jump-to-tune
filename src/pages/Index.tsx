...
function FeaturedArtistsSection() {
  ...
  const [artistStats, setArtistStats] = useState<Record<string, { trackCount: number; followerCount: number }>>({});
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      const stats: Record<string, { trackCount: number; followerCount: number }> = {};

      await Promise.all(
        featuredArtists.map(async (artist) => {
          const [tracksResult, followersResult] = await Promise.all([
            supabase
              .from("tracks")
              .select("id", { count: "exact", head: true })
              .eq("artist_id", artist.content_id)
              .eq("is_draft", false),
            supabase
              .from("follows")
              .select("id", { count: "exact", head: true })
